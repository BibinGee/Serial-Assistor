...
# Date: 2019-09-07
# combine function of start button and stop button
...
#         h4.addWidget(self.clearBtn)
        ...
        self.plotBtn.setText('Start')
        self.plotBtn.setFont(QFont("Microsoft YaHei"))
...
#             self.clearBtn.setVisible(False)
    ...
    # Function to switch Kidde mode and EWards mode
    # default mode: EWards mode
...
#             self.on_click_stop()
            ...
            self.EW_mode = True
            self.addBtn.setVisible(False)
#             self.clearBtn.setVisible(False)97
#             self.plotBtn.setVisible(False)
#             self.stopBtn.setVisible(False)
            self.list.setVisible(False)
            self.cnt_text.setVisible(False)
...
#             self.stopBtn.setVisible(True)
    ...
    ################################################################################
     ...
     # add key words to decide which data need to be plotted
...
#             print(self.keyWordsDict)
    ...
    # function to clear the data in dictionary
    # especially when mode switching, data must be clear before mode change
...
#         self.stopBtn.setEnabled(False)    
    ...
    # this function combines with 'start plotting' and 'stop plotting'
    
    ################################################################################
    def on_click_plot(self):
        if self.plotBtn.text() == 'Start':
            self.flag = True
            self.timer.start(400, self)
#             self.plotBtn.setEnabled(False)
#             self.stopBtn.setEnabled(True)
            self.plotBtn.setText('Stop')
        else:
            self.stopPlotting()
            self.plotBtn.setText('Start')
...
#         self.stopBtn.setEnabled(False)
    ...
    # function to parase the data from main window
    # Ewards mode and Kidde mode are in 2 different data combination
...
#             print(trimedString[0][:17])
    ...
    # function to update plotting with matplotlib figure
...
#                         print(self.x_data, self.keyWordsDict[key])
    ...
    # function to lopp ' update plotting' function every 400ms
